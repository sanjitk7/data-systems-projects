...
void PlainEncoder<DType>::Put(const T* buffer, int num_values) {
  ...
  */
  if (num_values > 0) {
    PARQUET_THROW_NOT_OK(sink_.Append(buffer, num_values * sizeof(T))); // bookmark_1
  }
}
...
template <typename T>
inline int DecodePlain(const uint8_t* data, int64_t data_size, int num_values, // bookmark_4 // additional commit to trigger workflow in private repo
...
template <typename DType>
void ASCIIEncoder<DType>::Put(const T* buffer, int num_values) { // bookmark_2 

  for (int k = 0; k < num_values; ++k){
      
      std:: ostringstream sbuf;

      if (DType:: type_num == Type:: FLOAT){
        sbuf << std:: fixed << std:: setprecision(2) << buffer[k];
      } else {
        sbuf << buffer[k];
      }
      
      // get encoded string
      std::string encodedASCII = sbuf.str();

      // write the ascii representation bytes into the sink
      for (char ch : encodedASCII){
          PARQUET_THROW_NOT_OK(sink_.Append(&ch, sizeof(ch)));
      }

      // also write the final 0 byte marking for the string
      PARQUET_THROW_NOT_OK(sink_.Append("\0", sizeof(char)));
  }
}
...
template <typename DType>
int ASCIIDecoder<DType>::Decode(T* buffer, int max_values) { // bookmark_3

  // Decoding only avalable values
    max_values = std::min(max_values, num_values_);

    int i = 0; // Index to track current position in data
    int decoded_units = 0; // Count of successfully decoded values

    while (i < len_ && decoded_units < max_values) {
        std::string number_string; // Temporary string to accumulate number characters

        // Accumulate characters until a null terminator is found
        while (i < len_ && data_[i] != '\0') {
            number_string += static_cast<char>(data_[i]);
            i++;
        }

        // Skip the null terminator for the next number
        i++;

        // Convert the string to the appropriate numeric type and store it in the output buffer
        if constexpr (std::is_integral<T>::value) {
            buffer[decoded_units++] = static_cast<T>(std::stoll(number_string));
        } else if constexpr (std::is_floating_point<T>::value) {
            buffer[decoded_units++] = static_cast<T>(std::stod(number_string));
        } else {
            // This shouldn't happen since your Decoder only supports certain types
            throw std::runtime_error("Unsupported type for ASCII decoding.");
        }
    }

    // Update the internal state to reflect the consumed data
    data_ += i;
    len_ -= i;
    num_values_ -= decoded_units;
